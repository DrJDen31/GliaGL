...
def test_network_wrapper():
    ...
    print(f"[OK] State property works")
    
    # Inject dict
    net.inject_dict({"S0": 100.0, "S1": 50.0})
    print(f"[OK] inject_dict() works")
    
    # Inject array
    net.inject_array(np.array([120.0, 80.0]))
    print(f"[OK] inject_array() works")
    
    # Multiple steps
    net.step(n_steps=10)
    print(f"[OK] step(n_steps) works")
    
    # Firing neurons
    firing = net.get_firing_neurons()
    print(f"[OK] get_firing_neurons(): {len(firing)} neurons fired")
...
def test_trainer_wrapper():
    ...
    print(f"[OK] Config property works")
    
    # History
    history = trainer.history
    assert isinstance(history, dict)
    print(f"[OK] History property works")
...
def test_dataset():
    ...
    print(f"[OK] Created dataset: {dataset}")
    
    # Length
    assert len(dataset) == 10
    print(f"[OK] len() works: {len(dataset)}")
    
    # Indexing
    ep = dataset[0]
    assert isinstance(ep, glia.EpisodeData)
    print(f"[OK] Indexing works")
    
    # Slicing
    subset = dataset[0:5]
    assert isinstance(subset, glia.Dataset)
    assert len(subset) == 5
    print(f"[OK] Slicing works")
    
    # Split
    train, val = dataset.split(train_fraction=0.7, shuffle=False)
    assert len(train) == 7
    assert len(val) == 3
    print(f"[OK] Split works: train={len(train)}, val={len(val)}")
    
    # Shuffle
    shuffled = dataset.shuffle(seed=42)
    assert len(shuffled) == len(dataset)
    print(f"[OK] Shuffle works")
...
def test_config_helpers():
    ...
    print(f"[OK] create_config() works")
    ...
    print(f"[OK] create_evo_config() works")
...
def test_file_io():
    ...
    try:
        net1.save(temp_path)
        print(f"[OK] Saved network to {temp_path}")
        
        # Load it back
        net2 = glia.Network.from_file(temp_path, verbose=False)
        print(f"[OK] Loaded network: {net2}")
        
        # Verify
        assert net2.num_neurons == net1.num_neurons
        print(f"[OK] Neuron counts match")
...
def test_evolution_wrapper():
    ...
    print(f"[OK] Evolution wrapper API exists")

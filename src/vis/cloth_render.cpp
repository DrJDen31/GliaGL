...
void Cloth::PackMesh() {
  ...
  if (mesh_data->force) {
    ...
    // new_cloth_tri_count += ????
    // *********************************************************************
      // (up to) 12 spring forces + 1 gravity force + net force + wind force
      new_cloth_tri_count += 15 * 12 * (nx) * (ny);
  }
  ...
  // *********************************************************************    
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            ...
            for (int k = 0; k < 12; k++) {
                addEdgeGeometry(current,
                    pos, pos + dt * 1000 * p.getForce(k),
                    Vec3f(1, 0, 1), Vec3f(0, 0, 1), thickness, thickness);
            }

            // gravity
            Vec3f gravity = getParticle(i, j).getMass() * Vec3f(mesh_data->gravity.data[0], mesh_data->gravity.data[1], mesh_data->gravity.data[2]);
            addEdgeGeometry(current,
                pos, pos + dt * 1000 * gravity,
                Vec3f(0, 1, 0), Vec3f(0, 1, 0), thickness, thickness);

            // net force
            addEdgeGeometry(current,
                pos, pos + dt * 1000 * p.getNetForce(),
                Vec3f(1, 0, 0), Vec3f(1, 0, 0), thickness, thickness);

            // wind force
            addEdgeGeometry(current,
                pos, pos + dt * 1000 * p.getWindForce(),
                Vec3f(0, 1, 1), Vec3f(1, 1, 1), thickness, thickness);
        }
    }
}
